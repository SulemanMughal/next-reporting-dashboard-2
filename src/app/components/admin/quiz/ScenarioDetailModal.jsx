...
function SubmitBtn({isSubmit,setShowScenario, submitHandler }){
    return (
        <>
            {isSubmit ? 
                ...
                >
                <FaPuzzlePiece size={23} className="mr-2"/> <span> Update </span> 
                </button> 
                </div>
                </>
                }
...
export default function ScenarioDetailModal({setShowScenario , setQuestion , quizId, question , setData}){
    const name = useRef("");
    const [tags, setTags] = useState([]);
    const description = useRef("");
    const difficulty = useRef("");
    const [isSubmit, setSubmit] = useState(false)
    const [data_2, setDataNew] = useState(null)
    useEffect(()=> {
        setDataNew(question?.scenario)
        if(data_2){
            name.current = data_2?.name
            description.current = data_2?.desc
    ...
    const submitHandler = async () => {
        ...
        if(name.current == ""  || description.current == ""  || difficulty.current == "" || tags.current == "" ){
            ...
            try {
                ...
                const res = await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/api/scenario/${data_2.id}`, {encryptedData});
                setShowScenario(false)
                const {...data } = decrypt(res.data.encryptedData)
                if(data.status === false){
                    toast.error(`Sorry, you can't update challenge. Please try again after sometime`)    
                } else {
                    toast.success('Successfull, Challenge has been created')
            ...
            <div className="relative w-1/3  px-4 space-y-16 ">
                ...
                <div className="space-y-6" >
                    ...
                    <div className="relative z-0 w-full my-6 group ">
                        <textarea id="Description" name="Description" rows="7" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" 
                    ...
                    {/* scenario field */}
                    <div className="relative z-0 w-full mb-6 group">
                        <select id="difficulty" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer "  
                ...
                <div>
                    <SubmitBtn isSubmit={isSubmit} setShowScenario={setShowScenario} submitHandler={submitHandler} />
                </div>
                </div>
                </div>
                </div>
