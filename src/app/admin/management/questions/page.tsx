...
export default function Page(){
    ...
    const DataFetch = () => {
        axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/management/questions/`)
        .then(res => {
            const {...fetched_data } = decrypt(res.data.encryptedData)
            if(fetched_data?.status === true){
                // console.debug(fetched_data?.challenges)
                setRowData(fetched_data?.questions);
            } else {
                setRowData([]);
            }
        })
            ...
            <div  className="p-4 mb-16">
                ...
                <GridComponent 
                    columnDefs={[ 
                        ...
                        { headerName: 'Statement', field: 'title' }, 
                        { headerName: 'Total Points', field: 'points' }, 
                        { headerName: 'Original Answer', field: 'original_answer' }, 
                        { headerName: 'Challenge', field: 'scenario.name' }, 
                        { headerName: 'Category', field: 'scenario.category' }, 
